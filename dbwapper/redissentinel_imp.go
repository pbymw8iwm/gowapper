...
func (r *RedisSentinelClient) Connect(param *RedisCacheParam) error {
	if len(param.Server_addrs) == 1 { //单机模式，否则是哨兵模式
		r.RedisClient = redis.NewClient(&redis.Options{
			Addr:     param.Server_addrs[0],
		...
		sc := redis.FailoverOptions{
			...
			SentinelAddrs: param.Server_addrs,
			Password:      param.Password,
			DB:            param.Dbindex,
...
func (client *RedisSentinelClient) Publish(channel string, message interface{}) int64 {
	return client.RedisClient.Publish(context.Background(), channel, message).Val()
}
